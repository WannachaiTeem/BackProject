...
router.get("/UID/:id", (req,res) => {
  conn.query("select * from Image where uid = ? ORDER BY rate DESC",
  [req.params.id], (err , result) => {
      if (err) {
          res.status(400).json(err);
      }
...
router.put("/:id", (req, res) => {
    ...
    let sqlvote = "delete from Vote where lid = ?";
    sqlvote = mysql.format(sqlvote, [
      id
  ]);
  conn.query(sqlvote, (err, result) => {
    if (err) throw err;
    // res
    //   .status(201)
    ...
    let sql =
      "update  `Image` set `img`=?, `name`=?,`rate`= 100   where `LID`=?";
    sql = mysql.format(sql, [
        updateimage.img,
        updateimage.name,
        id
...
router.delete("/delete/:id", (req, res) => {
    let id = +req.params.id;
    conn.query("delete from Vote where lid = ?", [id], (err, result) => {
       if (err) throw err;
