...
//ค้นหา user ทั้งหมด
router.get("/Ranking/get", (req, res) => {
  conn.query("SELECT COALESCE(v.latestScore, i.rate) AS latestScore, i.LID, DATE_FORMAT(DATE(voting_date), '%Y-%m-%d') AS voting_date, i.name, i.img FROM ( SELECT * FROM Image ORDER BY rate DESC LIMIT 10 ) AS i LEFT JOIN ( SELECT latestScore, LID, DATE_FORMAT(DATE(date), '%Y-%m-%d') AS voting_date FROM Vote WHERE (LID, VID) IN ( SELECT LID, MAX(VID) AS max_vid FROM Vote WHERE DATE(date) < CURDATE() GROUP BY LID )) AS v ON i.LID = v.LID ORDER BY COALESCE(v.latestScore, i.rate) DESC", (err, result, fields) => {
      res.json(result);
      console.log('success');
  });
});
...
// //ค้นหา user ทั้งหมด
// router.get("/Ranking/get", (req, res) => {
//   conn.query("SELECT COALESCE(v.latestScore, i.rate) AS latestScore, i.LID, DATE_FORMAT(DATE(voting_date), '%Y-%m-%d') AS voting_date, i.name, i.img FROM Image AS i LEFT JOIN (SELECT latestScore, LID, DATE_FORMAT(DATE(date), '%Y-%m-%d') AS voting_date FROM Vote WHERE (LID, VID) IN (SELECT LID, MAX(VID) AS max_vid FROM Vote WHERE DATE(date) < CURDATE() GROUP BY LID )) AS v ON i.LID = v.LID WHERE voting_date IS NOT NULL ORDER BY latestScore DESC", (err, result, fields) => {
//       res.json(result);
//       console.log('success');
//   });
// });
